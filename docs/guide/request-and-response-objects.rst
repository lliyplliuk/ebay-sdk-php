...
.. code-block:: php
    ...
    use \lliyplliuk\eBaySDK\Finding\Types;
...
Properties of the objects are named after the fields found in the offical eBay documentation. The SDK will throw a ``lliyplliuk\eBaySDK\Exceptions\UnknownPropertyException`` if you get or set a property that does not exist.
...
A ``lliyplliuk\eBaySDK\Exceptions\InvalidPropertyTypeException`` is thrown if a value of the wrong type is assigned to a property.
...
You can disable this type checking by assigning ``false`` to the static property ``\lliyplliuk\eBaySDK\Sdk::$STRICT_PROPERTY_TYPES``.
