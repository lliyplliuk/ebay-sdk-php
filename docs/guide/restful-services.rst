...
.. code-block:: php
    ...
    <?php
    use \lliyplliuk\eBaySDK\Browse\Types;

    $request = new Types\GetItemRestRequest();

    /**
     * $response will be of the type lliyplliuk\eBaySDK\Browse\Types\GetItemRestResponse
     */
...
The SDK provides some support for generating the OAuth tokens needed by the RESTful services. This is provided via the ``\lliyplliuk\eBaySDK\OAuth\Services\OAuthService`` class.


.. code-block:: php

    use \lliyplliuk\eBaySDK\OAuth\Services;
    use \lliyplliuk\eBaySDK\OAuth\Types;
