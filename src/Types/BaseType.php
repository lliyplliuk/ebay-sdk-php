...
use \lliyplliuk\eBaySDK\Types;
use \lliyplliuk\eBaySDK\Exceptions;
use \lliyplliuk\eBaySDK\JmesPath\Env;
use \lliyplliuk\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \lliyplliuk\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \lliyplliuk\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \lliyplliuk\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'lliyplliuk\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \lliyplliuk\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\lliyplliuk\eBaySDK\Types\BaseType', false)) {
